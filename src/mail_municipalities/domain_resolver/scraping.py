...
from mail_municipalities.domain_resolver.filtering import is_valid_tld
...
_RESERVED_DOMAINS = frozenset({"example.com", "example.org", "example.net", "test", "localhost", "invalid"})
...
async def _fetch_insecure(url: str) -> httpx.Response:
    ...
    with warnings.catch_warnings():
        ...
        async with httpx.AsyncClient(verify=False, http2=True) as insecure_client:
            return await insecure_client.get(url, follow_redirects=True, timeout=httpx.Timeout(30, connect=30))
...
async def validate_domain_accessibility(
    ...
    for prefix in [f"https://www.{domain}", f"https://{domain}"]:
        try:
            r = await client.head(prefix, follow_redirects=True, timeout=httpx.Timeout(10, connect=20))
...
async def scrape_email_domains(
    ...
    *,
    ...
    for path in all_paths:
        ...
        consecutive_failures = 0
        all_domains, redirect_domain = _process_scrape_response(r, domain, all_domains, redirect_domain, skip_domains)
        if all_domains and not exhaustive:
            return all_domains, redirect_domain, accessible
    ...
    *,
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            for base in [f"https://www.{bare}", f"https://{bare}"]:
                try:
                    response = await page.goto(base + "/", timeout=timeout_ms, wait_until="domcontentloaded")
...
def load_scrape_cache(path: Path, ttl_days: int = 7) -> dict[str, tuple[set[str], str | None, bool]]:
