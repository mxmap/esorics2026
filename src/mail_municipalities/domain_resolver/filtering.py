...
if TYPE_CHECKING:
    ...
    from mail_municipalities.domain_resolver.schemas import MunicipalityRecord
...
def filter_scraped_pool(
    ...
    if config.tlds:
        pool = {d for d in pool if any(d.endswith(t) for t in config.tlds) or d in candidate_domains}
