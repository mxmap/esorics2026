...
from mail_municipalities.domain_resolver.clients.openplz import fetch_openplz_municipalities
from mail_municipalities.domain_resolver.clients.static import load_bresu
    ...
    async def collect_candidates(self, data_dir: Path) -> list[MunicipalityRecord]:
        ...
        for gkz in sorted(all_gkz):
            ...
            if bresu_ed and bresu_ed.lower() not in self.skip_domains:
                d = bresu_ed.lower()
                rec.candidates.append(DomainCandidate(domain=d, source="bresu_email", is_email_domain=True))
