...
from mail_municipalities.domain_resolver.clients.bfs import (
    CANTON_SHORT_TO_FULL,
    fetch_bfs_municipalities,
)
from mail_municipalities.domain_resolver.clients.openplz import fetch_openplz_ch_municipalities
...
class SwitzerlandConfig(CountryConfig):
    ...
    async def collect_candidates(self, data_dir: Path) -> list[MunicipalityRecord]:
        ...
        if openplz_only:
            details = ", ".join(f"{c} ({openplz[c]['name']})" for c in sorted(openplz_only, key=int))
            logger.warning(
        ...
        if bfs_only_codes:
            details = ", ".join(f"{c} ({bfs_municipalities[c]['name']})" for c in sorted(bfs_only_codes, key=int))
            logger.warning(
        ...
        for bfs, bfs_entry in sorted(bfs_municipalities.items(), key=lambda kv: int(kv[0])):
            ...
            if not canton:
                ...
                if plz_entry and plz_entry.get("canton"):
                    ...
                    logger.debug("Using OpenPLZ canton for {} ({}): {}", bfs, bfs_entry["name"], canton)
            if not canton:
