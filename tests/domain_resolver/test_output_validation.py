...
DOMAINS_DIR = Path(__file__).resolve().parent.parent.parent / "output" / "domains"
...
def assert_record_invariants(rec: MunicipalityRecord) -> None:
    ...
    if rec.emails:
        assert rec.confidence != Confidence.NONE, f"{rec.code} {rec.name}: has emails {rec.emails} but confidence=NONE"
    ...
    if rec.source == Source.NONE:
        assert rec.confidence == Confidence.NONE, f"{rec.code} {rec.name}: source=NONE but confidence={rec.confidence}"
    ...
    if "no_mx" in rec.flags:
        assert rec.source == Source.OVERRIDE, f"{rec.code} {rec.name}: no_mx flag but source={rec.source}"
    if "guess_only" in rec.flags:
        assert rec.source == Source.GUESS, f"{rec.code} {rec.name}: guess_only flag but source={rec.source}"
    ...
    if rec.website_domain:
        assert "." in rec.website_domain, f"{rec.code} {rec.name}: website '{rec.website_domain}' missing TLD"
...
class TestExportOutputValidation:
    ...
    def test_minimal_output_structure(self, ch_records, tmp_path):
        ...
        data = json.loads((tmp_path / "domains_ch.json").read_text())
    ...
    def test_detailed_output_structure(self, ch_records, tmp_path):
        phase_export(ch_records, tmp_path, "ch")
        data = json.loads((tmp_path / "domains_ch_detailed.json").read_text())
    ...
    def test_review_only_contains_flagged_or_low_confidence(self, ch_records, tmp_path):
        phase_export(ch_records, tmp_path, "ch")
        data = json.loads((tmp_path / "domains_ch_review.json").read_text())
...
class TestExportMultiCountry:
    ...
    def test_varied_scenarios_consistent(self, cc, config_cls, make_fn, tld, tmp_path):
        ...
        for suffix in [
            f"domains_{cc}.json",
            f"domains_{cc}_detailed.json",
            f"domains_{cc}_review.json",
        ]:
            data = json.loads((tmp_path / suffix).read_text())
            assert data["total"] >= 0
            assert len(data["municipalities"]) == data["total"]

        detailed = json.loads((tmp_path / f"domains_{cc}_detailed.json").read_text())
...
def _load_output(cc: str, suffix: str = "") -> dict | None:
    ...
    filename = f"domains_{cc}{suffix}.json"
...
class TestRealOutputFiles:
    ...
    def test_minimal_structure(self, cc):
        ...
        if data is None:
            pytest.skip(f"domains_{cc}.json not found")
    ...
    def test_detailed_structure_and_invariants(self, cc):
        ...
        if data is None:
            pytest.skip(f"domains_{cc}_detailed.json not found")
    ...
    def test_review_only_flagged_or_low(self, cc):
        ...
        if data is None:
            pytest.skip(f"domains_{cc}_review.json not found")
    ...
    def test_regions_are_valid(self, cc, expected_regions):
        ...
        for m in data["municipalities"]:
            ...
            assert m["region"] in expected_regions, f"[{cc}] {m['code']} {m['name']}: unknown region '{m['region']}'"
    ...
    def test_review_is_subset_of_detailed(self, cc):
        ...
        if detailed is None or review is None:
            pytest.skip(f"domains_{cc}_detailed.json or {cc}_review.json not found")
    ...
    def test_minimal_and_detailed_agree(self, cc):
        ...
        if minimal is None or detailed is None:
            pytest.skip(f"domains_{cc}.json or {cc}_detailed.json not found")
