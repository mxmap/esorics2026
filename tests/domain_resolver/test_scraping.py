...
class TestExtractEmailDomains:
    ...
    def test_typo3_url_encoded(self):
        ...
        html = '<a href="javascript:linkTo_UnCryptMailto(%27kygjrm8amkkslc%2CzmsbpwYlc%2Caf%27);">Contact</a>'
        domains = extract_email_domains(html, set())
    ...
    def test_data_email_link_base64(self):
        html = '<a data-email-link="bWFpbHRvJTNBaW5mbyU0MDM3MTUlMkVjaA==" rel="nofollow">Contact</a>'
        domains = extract_email_domains(html, set())
        assert "3715.ch" in domains

    def test_null_span_injection(self):
        html = '<a href="#">info<span class="none" data-nosnippet="" aria-hidden="true">NULL</span>@example.ch</a>'
        domains = extract_email_domains(html, set())
        assert "example.ch" in domains

    def test_nanmail_span_injection(self):
        html = '<a class="nanmail">mail@<span class="d2024-02-14" style="display: none;">@@null</span>belmont.ch</a>'
        domains = extract_email_domains(html, set())
