...
from mail_municipalities.domain_resolver.clients.bfs import BFS_API_URL, fetch_bfs_municipalities
from mail_municipalities.domain_resolver.clients.openplz import (
    OPENPLZ_BASE_AT,
    OPENPLZ_BASE_CH,
    fetch_openplz_ch_municipalities,
    fetch_openplz_municipalities,
)
from mail_municipalities.domain_resolver.clients.wikidata import SPARQL_URL, fetch_wikidata
...
class TestFetchBfsMunicipalities:
    async def test_fetch_and_parse(self):
        ...
        with respx.mock:
            respx.get(BFS_API_URL).respond(200, text=csv_text)
    ...
    async def test_fetch_and_parse(self):
        with respx.mock:
            ...
            for canton_key in range(2, 27):
                respx.get(f"{OPENPLZ_BASE_CH}/{canton_key}/Communes?page=1&pageSize=50").respond(200, json=[])
    ...
    async def test_fetch_paginated(self):
        with respx.mock:
            ...
            for bl in range(2, 10):
                respx.get(f"{OPENPLZ_BASE_AT}/{bl}/Municipalities?page=1&pageSize=50").respond(200, json=[])
