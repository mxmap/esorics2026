...
from mail_municipalities.domain_resolver.pipeline import (
    _print_dry_run,
    _set_website,
...
class TestPhaseDnsPrefilter:
    async def test_filters_nonresolving_domains(self):
        ...
        with patch("mail_municipalities.domain_resolver.pipeline.lookup_a", side_effect=_lookup_a):
    ...
    async def test_keeps_all_resolving(self):
        ...
        with patch(
            "mail_municipalities.domain_resolver.pipeline.lookup_a",
    ...
    async def test_uses_dns_cache(self, tmp_path):
        records = [_make_record(candidates=[DomainCandidate(domain="cached.de", source="livenson")])]
    ...
    async def test_www_fallback_resolves(self):
        ...
        records = [_make_record(candidates=[DomainCandidate(domain="www-only.ch", source="wikidata")])]
...
class TestPhaseContentValidate:
    async def test_classifies_parked_domain(self):
        records = [_make_record(candidates=[DomainCandidate(domain="parked.de", source="livenson")])]
        config = GermanyConfig()
        validation: dict[str, tuple[bool, str | None, bool]] = {"parked.de": (True, None, False)}

        with respx.mock:
            respx.get("https://www.parked.de/").respond(200, html="<html>This domain is parked</html>")
    ...
    async def test_classifies_municipality_page(self):
        records = [_make_record(candidates=[DomainCandidate(domain="gemeinde.de", source="livenson")])]
        config = GermanyConfig()
        validation: dict[str, tuple[bool, str | None, bool]] = {"gemeinde.de": (True, None, False)}

        with respx.mock:
            respx.get("https://www.gemeinde.de/").respond(200, html="<html><h1>Gemeinde Musterstadt</h1></html>")
            flags = await phase_content_validate(records, config, validation)
