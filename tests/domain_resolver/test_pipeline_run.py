...
class TestRunPipeline:
    ...
    async def test_full_run_mocked(self, tmp_path):
        ...
        with (
            patch(
                "mail_municipalities.domain_resolver.pipeline.phase_dns_prefilter",
                new_callable=AsyncMock,
                return_value={"test.de": True},
            ),
            patch(
                "mail_municipalities.domain_resolver.pipeline.phase_validate",
                new_callable=AsyncMock,
                return_value={"test.de": (True, None, False)},
            ),
            patch(
                "mail_municipalities.domain_resolver.pipeline.phase_scrape",
                new_callable=AsyncMock,
                return_value={"test.de": ({"test.de"}, None, True)},
            ),
            patch(
                "mail_municipalities.domain_resolver.pipeline.phase_mx",
                new_callable=AsyncMock,
                return_value={"test.de": True},
            ),
        ):
        ...
        assert (tmp_path / "out" / "domains_de.json").exists()
        data = json.loads((tmp_path / "out" / "domains_de.json").read_text())
        assert data["total"] == 1
