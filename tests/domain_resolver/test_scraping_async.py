...
class TestValidateDomainAccessibility:
    async def test_accessible(self):
        with respx.mock:
            ...
            async with httpx.AsyncClient() as client:
                accessible, redirect, ssl_failed = await validate_domain_accessibility(client, "example.ch")
    ...
    async def test_redirect(self):
        with respx.mock:
            ...
            async with httpx.AsyncClient(follow_redirects=True) as client:
                accessible, redirect, ssl_failed = await validate_domain_accessibility(client, "old.ch")
        assert accessible is True
    ...
    async def test_not_accessible(self):
        with respx.mock:
            ...
            async with httpx.AsyncClient() as client:
                accessible, redirect, ssl_failed = await validate_domain_accessibility(client, "dead.ch")
    ...
    async def test_connection_error(self):
        with respx.mock:
            ...
            async with httpx.AsyncClient() as client:
                accessible, redirect, ssl_failed = await validate_domain_accessibility(client, "nohost.ch")
        assert accessible is False
...
class TestScrapeEmailDomains:
    async def test_finds_emails(self):
        with respx.mock:
            ...
            respx.get("https://www.example.ch/kontakt").respond(200, html="<p>admin@example.ch</p>")
            respx.get("https://example.ch/").respond(200, html="")
            respx.get("https://example.ch/kontakt").respond(200, html="")

            async with httpx.AsyncClient() as client:
                emails, redirect, accessible = await scrape_email_domains(client, "example.ch", ["/kontakt"], set())
    ...
    async def test_respects_skip_domains(self):
        with respx.mock:
            respx.get("https://www.example.ch/").respond(200, html="<p>info@example.ch user@gmail.com</p>")
            respx.get("https://example.ch/").respond(200, html="")
    ...
    async def test_consecutive_failure_bailout(self):
        with respx.mock:
            ...
            respx.get("https://www.failing.ch/kontakt").mock(side_effect=httpx.ConnectError("refused"))
            respx.get("https://www.failing.ch/impressum").mock(side_effect=httpx.ConnectError("refused"))
    ...
    async def test_detects_redirect(self):
        with respx.mock:
            ...
            async with httpx.AsyncClient() as client:
                emails, redirect, accessible = await scrape_email_domains(client, "old.ch", [], set())
...
class TestValidateDomainAccessibilitySSL:
    async def test_ssl_error_insecure_fallback_succeeds(self):
        ...
        with patch("mail_municipalities.domain_resolver.scraping.httpx.AsyncClient", return_value=mock_ic):
            accessible, redirect, ssl_failed = await validate_domain_accessibility(outer_client, "sslbad.ch")
    ...
    async def test_generic_exception_continues(self):
        ...
        with respx.mock:
            ...
            async with httpx.AsyncClient() as client:
                accessible, redirect, ssl_failed = await validate_domain_accessibility(client, "weird.ch")
        assert accessible is True
        assert ssl_failed is False
...
class TestTryFetch:
    async def test_ssl_failed_flag_insecure_succeeds(self):
        ...
        with patch(
            "mail_municipalities.domain_resolver.scraping._fetch_insecure",
...
class TestScrapeEmailDomainsExtended:
    async def test_discovers_contact_links_from_homepage(self):
        ...
        with respx.mock:
            ...
            respx.get("https://www.example.ch/kontakt-formular").respond(200, html="<p>info@example.ch</p>")

            async with httpx.AsyncClient() as client:
                emails, redirect, accessible = await scrape_email_domains(client, "example.ch", [], set())
    ...
    async def test_subpage_failure_then_success(self):
        ...
        with respx.mock:
            ...
            respx.get("https://www.example.ch/kontakt").mock(side_effect=httpx.ConnectError("refused"))
            respx.get("https://www.example.ch/impressum").respond(200, html="<p>info@example.ch</p>")
    ...
    async def test_non_exhaustive_stops_on_subpage(self):
        ...
        with respx.mock:
            ...
            respx.get("https://www.example.ch/impressum").respond(200, html="<p>other@example.ch</p>")
