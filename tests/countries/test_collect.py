...
def _make_de_data(tmp_path: Path):
    ...
    b42labs = {
        ...
        "total": 1,
        "municipalities": {
            "01001000": {"bfs": "01001000", "name": "Flensburg", "domain": "flensburg.de"}
        },
    }
...
class TestGermanyCollect:
    async def test_basic(self, tmp_path):
        ...
        with patch(
            "municipality_email.countries.germany.fetch_wikidata",
            new_callable=AsyncMock,
            return_value=wikidata_data,
        ):
            records = await config.collect_candidates(data_dir)
...
class TestAustriaCollect:
    async def test_basic(self, tmp_path):
        ...
        with (
            patch(
                "municipality_email.countries.austria.fetch_openplz_municipalities",
                new_callable=AsyncMock,
                return_value=openplz_data,
            ),
            patch(
                "municipality_email.countries.austria.fetch_wikidata",
                new_callable=AsyncMock,
                return_value=wikidata_data,
            ),
        ):
